function obj = FindHFOTimeStamps80(obj)

D = obj.iEEGBandPass80;
Thresholds = obj.ThresholdsALL80;
OnOffsetThreshold = Thresholds/obj.SDthreshold*3;

% initialize the output
obj.CandidateHFOsTimeStamps80 = cell(length(D.chanlabels),1);
...
for i = 1:nChannel
    ...
    obj.CandidateHFOsTimeStamps80{i} = HFO_Events_Matrix;
end

end

% % Write the time stamps
% EventTime = HFO_timestamps{80}(:,1)/D.fsample;
% EventMarker = repmat('HFO80',[length(EventTime),1]);
% eventFileName = ['Detected_80.txt'];
% Duration = (HFO_timestamps{80}(:,3) - HFO_timestamps{80}(:,1))/D.fsample
