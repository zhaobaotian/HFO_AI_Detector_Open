...
subPath      = 'E:\3.ClinicalHFO\RenBin';
%
...
% Load data
Elec.LoadAnat;
Elec.LoadElectrodeNameFile;
Elec.LoadElectrodePosFile;
Elec.ReadData;
try
...
Elec.ExtractBrain;
disp('-------------Brain extracted-----------')
Elec.BinaryBrainMask;
Elec.LabelOutBrainElectrodes;
disp('-----Out brain electrodes labeled------')
Elec.MakeGreyMatterMask;
Elec.LabelWhiteMatterElectrodes;
% Process iEEG
Elec.iEEGPreprocessingRMS;
Elec.RMSCalcThreshold;
disp('----White matter electrodes labeled----')
...
%% Stage2: HFOs detection and classification
HFO = HFOs;
...
HFOV.ElectrodeNameFile       = Elec.ElectrodeNameFile;
HFOV.ElectrodePosFile        = Elec.ElectrodePosFile;
HFOV.AnatFile                = Elec.AnatFile;
HFOV.BipolarElectrodeName    = Elec.BipolarElectrodeName;
HFOV.BipolarInclusion        = Elec.BipolarInclusion;
HFOV.SelectedBipolarChannels = Elec.BipolarElectrodeName(Elec.BipolarInclusion);
...
% Plot the results in nii format
% Electrode coverage mask
HFOV.MakeBipolarElectrodeMask;

% HFOs counts
HFOV.WriteNIfTICandidates;
HFOV.WriteNIfTIqHFOs;
HFOV.WriteNIfTIqFRHFOs;
disp('-------------NIfTIs Written------------')
% Smooth the results
HFOV.Smooth;
disp('-------------NIfTIs smoothed-----------')
% Normalize the results
HFOV.SpatialNorm;
disp('-----------NIfTIs normalized-----------')
% Save results
HFOV.SaveResults
